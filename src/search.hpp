...
std::vector<Algorithm> depth_first_search(const Node<Cube> &root, const MoveTable &m_table, const PruningTable &p_table, const uint max_depth = 4) {
    ...
    while (queue.size() > 0) {
        ...
        if (node.state.is_solved()) {
            ...
            if (node.depth + node.estimate <= max_depth){
                auto children = node.expand(
                    apply, heuristic, allowed_next(node.sequence.back())
                );
                for (auto&& child : children) {
